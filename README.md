...
| 로그인 화면 | 홈 화면 | 달력 화면 | 프로필 화면 |
|:-:|:-:|:-:|:-:|
| 🔐 로그인·회원가입 | 🌰→🌲 식물 성장 | 📅 기분별 달력 | 👤 통계·업적 |

## 구현된 화면

### 로그인 (LoginScreen)
- 이메일 + 비밀번호로 로그인 / 회원가입 (탭 전환)
- 닉네임 설정 (회원가입 시)
- 게스트로 시작하기 (로그인 없이 이용)
- 입력 유효성 검사 + 에러 배너
- 로그인 상태 Hive 영구 저장 (앱 재시작 시 자동 복원)
...
- 설정: 닉네임 변경, 전체 기록 삭제, 로그아웃

## 핵심 기능

- **로그인 / 회원가입** — 이메일·비밀번호 기반, Hive 로컬 저장
- **게스트 모드** — 로그인 없이 즉시 시작, 언제든 전환 가능
- **로그아웃** — 프로필 화면에서 로그아웃 (데이터 유지)
